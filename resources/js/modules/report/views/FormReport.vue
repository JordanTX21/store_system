<template>
  <div class="card">
    <div class="card-header">
      <h3 class="mb-0">Registro del nuevo report</h3>
    </div>
    <div class="card-body">
  ...
  methods: {
    ...
    async sendEditData() {
      ...
      try {
        ...
        const result = await axios.put(`/url-report/${body.id}`, { ...body }).then(async (result) => {
    ...
    async sendCreateData() {
      ...
      try {
        ...
        const result = await axios.post('url-report', { ...body });
        if (result.status === 200) {
          if (!result.data.success) {
            await Alerts.showToastErrorMessage(result.data.message);
            return;
