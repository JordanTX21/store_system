    ...
    methods: {
        ...
        async sendEditData(status) {
            ...
            try {
                ...
                const result = await axios.put(`/solicitude/${body.id}`, { ...body }).then(async (result) => {
                    if (result.status === 200) {
                        ...
                        this.$router.push({ name: 'listsolicitude' })
                    }
                }).catch((err) => {
        ...
        async sendCreateData() {
            ...
            try {
                ...
                const result = await axios.post('solicitude', { ...body });
                if (result.status === 200) {
                    if (!result.data.success) {
                        await Alerts.showToastErrorMessage(result.data.message);
                        return;
        ...
        validateStatus() {
            if (this.status === 'EDIT') {
                ...
                if (this.$route.name === 'updatesolicitude' && this.item === undefined) {
                    this.$router.push({ name: 'listsolicitude' }) //newsolicitude
                }
            }
        }
    },
